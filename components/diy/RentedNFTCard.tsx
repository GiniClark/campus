...
export const RentedNFTCard: React.FC<RentedNFTCardProps> = ({ nft, onRentalEnded }) => {
  ...
  const calculateRemainingTime = () => {
    try {
      ...
      const startTime = Number(nft.rentalInfo.startTime) * 1000; // 转换为毫秒
      const duration = Number(nft.rentalInfo.duration) * 1000; // 转换为毫秒
  ...
  const handleEndRental = async () => {
    ...
    try {
      ...
      notification.error({ message: "归还 NFT 失败"});
    }
  };
